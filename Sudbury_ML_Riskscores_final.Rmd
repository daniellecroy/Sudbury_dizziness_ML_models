---
title: "Machine Learning Ohle Risk Score"
...
```{r}
#Load Packages
Packages <- c("readxl", "tree", "dplyr", "rpart", "MLmetrics", "caret", "xgboost", "rBayesianOptimization", "mltools", "data.table", "randomForest", "performanceEstimation", "ROSE", "gbm", "caret", "glmnet", "epiR")
...
full_dizzy<-full_join(train_ful2,test_60_2)
...
table1
write_lines(table1, "vertigo_ML_table1.html", append=F)
...
table2
write_lines(table2, "vertigo_ML_table2.html", append=F)
```

#Balancing Data
```{r}
set.seed(360)
table(train_full$outcome)
...
#Confirm best vcalue of mtry
df_predictors_train<-as.data.frame(df_predictors_train)
df_predictors_train$outcome<-as.factor(df_predictors_train$outcome)
bestmtry<-tuneRF(df_predictors_train[,-18], df_predictors_train[,18], stepFactor=0.7, improve=1e-5, ntree=500, mtryStart=5)
print(bestmtry)
...
#Get predicted probabilities from training data
prob_train<-predict(rf_riskscore, newdata=df_predictors_train, type="prob")
predicted_class<-ifelse(prob_train[,"yes"]>0.20, "yes", "no")
confusionMatrix(as.factor(predicted_class), as.factor(df_predictors_train$outcome), positive="yes")
...
rf_data<-as.table(matrix(c(30,166,0,1172), nrow=2, byrow=T))
...
varImpPlot(rf_riskscore, type=1)
...
rf_full_data<-as.table(matrix(c(215,734,2,5688), nrow=2, byrow=T))
...
set.seed(746)
xgb_model<-train(x=as.matrix(df_train_xgb2), y=as.factor(df_train_xgb$outcome_xgb), method="xgbTree", trControl=train_control2, tuneGrid=tune_grid2, metric="Accuracy")
print(xgb_model)
...
gb_data<-as.table(matrix(c(28,145,2,1193), nrow=2, byrow=T))
...
gb_full_data<-as.table(matrix(c(213,752,2,5670), nrow=2, byrow=T))
...
lasso_data<-as.table(matrix(c(30,301,0,1037), nrow=2, byrow=T))
lasso_data
...
lasso_full_data<-as.table(matrix(c(214,1486,1,4936), nrow=2, byrow=T))
lasso_full_data
...
rpart.plot(tree_final, type=3, extra=102, fallen.leaves = T)
...
summary(riskscore_model1)
prob=predict(riskscore_model1, type=c("response"))
predi<-prediction(prob, test_score$outcome2)
